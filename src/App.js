...
const inputStyle = {
  ...
  padding: "10px 12px", color: "#e8eaf0", fontSize: 14,
...
async function loadFromDB() {
  const { data, error } = await supabase.from("items").select("data").eq("id", ROW_ID).single();
...
function Field({ label, children }) {
  return (
    <div>
      <div style={{ fontSize: 12, color: "#8892a4", marginBottom: 6, fontWeight: 500 }}>{label}</div>
      {children}
    </div>
  );
}
    ...
    <div style={{ minHeight: "100vh", background: "#0a0f1e", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'DM Sans','Helvetica Neue',sans-serif", padding: 24 }}>
      <div style={{ background: "#111827", borderRadius: 20, padding: "40px 32px", width: "100%", maxWidth: 360, border: "1px solid rgba(255,255,255,0.08)", textAlign: "center" }}>
        ...
        <input
          ...
          style={{ ...inputStyle, borderColor: fout ? "#ff4444" : "rgba(255,255,255,0.1)", marginBottom: 8 }}
        />
...
export default function App() {
  ...
  if (!ingelogd) {
    return <LoginScreen onLogin={() => { sessionStorage.setItem("fk_auth", "1"); setIngelogd(true); }} />;
  }
...
function FrostKeep() {
  ...
  const filtered = items
    ...
    .filter(i => {
      if (filterStatus === "Alles") return true;
      return getExpiryStatus(getDaysUntilExpiry(i.expiryDate)) === statusFilterMap[filterStatus];
    })
          ...
          <>
            ...
            <div style={{ display: "flex", gap: 6, marginBottom: 20, flexWrap: "wrap" }}>
              {["Alles", "Vers", "Goed", "Bijna verlopen", "Verlopen"].map(s => {
                const k = statusFilterMap[s];
                const sc = k !== "Alles" ? STATUS_CONFIG[k] : null;
                return (
